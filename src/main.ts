...
class PossessionScene extends Phaser.Scene {
  ...
  updateAngelGuidance() {
    ...
    this.directionalAngels.forEach(statue=>{
      if(!target){
        statue.setAngle(0).setTint(0x776f72)
        return
      }
      const angle=Phaser.Math.Angle.Between(statue.x,statue.y,target.x,target.y)
      // 원본 석상의 손가락은 위쪽을 향하므로 목표 각도에 90도를 더한다.
      statue.setAngle(Phaser.Math.RadToDeg(angle)+90).setTint(0xc8e5df)
    })
  }
  ...
  createStoryIntro() {
    const pages=[
      ...
      {
        chapter:'V · 공포',
        title:'악마도 성기사를 두려워한다',
        body:'악마들은 무의식 깊은 곳에서 성기사를 두려워한다.\n특히 자신의 공격이 신성한 무기에 막히는 순간, 그 공포는 선명해진다.\n문지기의 공포를 끝까지 채우면 무릎을 꿇릴 수 있다.\n그러나 한 번 쓰러진 문지기는 분노하여 두 육체로 갈라질 것이다.'
      }
    ]
    let page=0
    const bg=this.add.rectangle(640,360,780,520,0x0d0e12,.97).setStrokeStyle(2,0x7b3035)
